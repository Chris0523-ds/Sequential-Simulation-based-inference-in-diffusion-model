...
sbibm_obs_number_list = [1]
simulation_budget_list = [1000]
batch_size_list = [None]
lr_list = [1e-4]
num_rounds_list = [1,2] # num_rounds_list = [1,10]
sde_name_list = ["vesde"]#, "vpsde"]
epsilon_list = [1e-3]
max_iters_list = [3000]
print("the dataset list includes:",dataset_list)
print("the sbibm_obs_number list is:",sbibm_obs_number_list)
list_of_lists = [
    dataset_list,
...
print("the length of parameter_df_full:",len(parameter_df_full))

for jobid in range(len(parameter_df_full)):
    jobid = int(jobid)
    ...
    print("the type of sbibm_obs: ", type(sbibm_obs_number))
    config = get_default_configs(dataset = dataset, num_rounds=num_rounds, obs_number=sbibm_obs_number,simulation_budget=np.int64(simulation_budget)) #0,1,5
    config.algorithm.compute_c2st_intermediate_rounds = False
    config.algorithm.simulation_budget = np.int64(simulation_budget)  #2
    ...
    print("The simulation_budget now is: ",simulation_budget)
    print("The dataset now is: ", dataset)
    print("The sde name is: ", sde_name)
    print("The obs_number is: ", sbibm_obs_number)
