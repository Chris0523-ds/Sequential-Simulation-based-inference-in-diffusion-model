...
def train_score_network(config, model, sde, parameter_ds, data_ds, key):
    # 设置日志配置，确保打印的信息写入文件
    logging.basicConfig(
        filename=config.output_file,  # 指定输出日志的文件
        level=logging.INFO,           # 设置日志级别
        format='%(message)s'          # 定义日志格式
    )
    ...
    for step in range(config.optim.max_iters):  
        ...
        if math.isnan(eval_loss.item()) or math.isnan(train_loss.item()):
            ...
            logger.info(f"Step={step}: training loss={round(total_train_loss / total_train_size,3)}, evaluation loss={round(total_eval_loss / total_eval_size,3)}, patience = {patience}")
            logger.info("NaN loss encountered. Aborting training.")
            break
            break
        ...
        if eval_loss_avg < best_loss:
            ...
            if patience > config.optim.max_patience:
                ...
                logger.info(f"Early stopping at step {step} with evaluation loss {round(best_loss,3)}")
                break
        ...
        if (step % config.optim.print_every) == 0 or (step == config.optim.max_iters - 1):
            ...
            logger.info(f"Step={step}: training loss={round(train_loss_avg,3)}, evaluation loss={round(eval_loss_avg,3)}, patience = {patience}, best_loss={round(best_loss,3)}")
        total_train_loss = 0
        total_train_size = 0
        total_eval_loss = 0
        total_eval_size = 0
